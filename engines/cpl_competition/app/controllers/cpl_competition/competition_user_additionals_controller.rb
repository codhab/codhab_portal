module CplCompetition
  class CompetitionUserAdditionalsController < ApplicationController
    skip_before_filter :verify_authenticity_token  
    before_action :set_competition_user

    def create
      @document = @user_competition.competition_user_participations.where(partipation_type: 2).new(set_params)
    ...
    def destroy
      @document = @user_competition.competition_user_participations.where(partipation_type: 2).find(params[:id])
    ...
    def show
      @documents = @user_competition.competition_user_participations.where(partipation_type: 2, deleted: false)
    end
